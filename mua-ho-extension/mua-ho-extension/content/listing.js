...
(function () {
  ...
  const PRODUCT_LINK =
    'a[href*="detail.1688.com/offer/"], a[href*="item.taobao.com/item.htm"], a[href*="item.htm?id="], a[href*="detail.tmall.com/item.htm"], a[href*="/offer/"][href*=".html"], a[href*="alibaba.com/product-detail/"]';
  ...
  function parseLink(href) {
    ...
    if (!id && href.includes("alibaba.com/product-detail/")) {
      m = href.match(/_(\d{6,})\.html/) || href.match(/\/(\d{6,})\.html/);
      if (m) { source = "alibaba"; id = m[1]; url = href.split("#")[0].split("?")[0]; }
    }
    if (!id) {
      m = href.match(/[?&]id=(\d+)/);
      if (m) {
